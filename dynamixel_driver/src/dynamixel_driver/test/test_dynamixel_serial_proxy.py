...
_except_msg = '[Fatal Error: servo #{} on {}@{}bps]: {} failed'.format(
    _FAKE_SERVO, _FAKE_PORT, _FAKE_BAUDRATE, _FAKE_COMMAND_FAILED)


def get_dynamixel_io_mock():
    mock = MagicMock(spec=dxio.DynamixelIO)
    mock.get_feedback.side_effect = dxio.FatalErrorCodeError(_except_msg, 32)
    return mock
...
if __name__ == '__main__':
    ...
    import rostest
    rostest.rosrun(PKG,
                   'test_dynamixel_serial_proxy',
                   TestDynamixelSerialProxy)
